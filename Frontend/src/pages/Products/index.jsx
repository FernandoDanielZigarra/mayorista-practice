...
function Products() {
  const { data, isPending } = useCustomFetch(`${import.meta.env.VITE_BASE_URL}/api/v1/products`);
  return (
    <main className="flex flex-col bg-mariner-900 rounded-none w-full min-h-[76vh]">
      ...
      <div className="rounded-t-3xl pt-5 w-full shadow-[inset_0px_4px_13px_4px_#00000024] bg-slate-100 pb-5 min-h-[53vh]">
        {isPending && <Spinner />}
        {!isPending && <ProductsList products={data} />}
      </div>
    </main>
  )
}
