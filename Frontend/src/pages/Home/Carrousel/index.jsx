...
const images = [
  {
    id: 1,
    url: "https://media.eldiariosur.com/p/00124fcafda3c97ae1fda672ebabd89b/adjuntos/291/imagenes/000/495/0000495645/1200x675/smart/ventas-mayoristasjpg.jpg",
  },
  {
    id: 2,
    url: "https://www.telam.com.ar/thumbs/bluesteel/advf/imagenes/2014/12/5499a664b30dd_450.jpg",
  },
  {
    id: 3,
...
function Carrousel() {
  ...
  const scrollToImage = (direction) => {
    if (direction === "prev") {
      setCourrentIndex((curr) => {
        const isFirstSlide = courrentIndex === 0;
        return isFirstSlide ? 0 : curr - 1;
      });
    } else {
      const isLastSlide = courrentIndex === images.length - 1;
      if (!isLastSlide) {
        setCourrentIndex((curr) => curr + 1);
      }
    }
  };
  ...
  return (
    <div className="w-screen h-[calc(100vh-8.5rem)]">
        ...
        <div className="w-full h-[100%] overflow-hidden">
          <ul className="list-none whitespace-nowrap" ref={listRef}>
            {images.map((image) => (
              <li
                key={image.id}
                className="whitespace-nowrap list-none inline-block"
              >
                <img
                  src={image.url}
                  alt=""
                  className="w-screen h-[calc(100vh-8.5rem)] object-cover"
                />
              </li>
            ))}
          </ul>
